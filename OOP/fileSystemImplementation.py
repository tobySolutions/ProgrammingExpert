...
- You are to create a file system that has Directories (folders) and Files each of which are Nodes. Validate paths, check for inconsistencies with path names in the Tree structure and ensure to go to the Penultimate node in the path before performing any operations to ensure that things are handled correctly.

- Problem prompt is on ProgrammingExpert. Read code to better understand. I didn't include too many comments ti ensure that the code is readable. I hope this helps
...
class FileSystem:
    ...
    def size(self):
        ...
        # We'll use a stack to keep track of files and folders unchecked
        while len(nodes) > 0:
            ...
            if isinstance(current_node, File):
                size += len(current_node.contents)
        return size
